...
interface TopSellingGemTypesProps {
  data?: GemTypeData[];
}

export function TopSellingGemTypes({ data }: TopSellingGemTypesProps) {
    ...
    <div className="rounded-xl border border-border bg-card p-5 h-full sass-enter gem-fade-in">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-cyan-500/10 text-cyan-500 dark:text-cyan-400">
            <Gem className="h-4 w-4" />
          </div>
          <div>
            <h2 className="text-sm font-semibold text-foreground">Top Gem Types</h2>
            <p className="text-xs text-muted-foreground">Best selling gemstones</p>
          </div>
        </div>
        <Link
          href="/reports"
